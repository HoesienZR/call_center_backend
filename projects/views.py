...
from call_center.utils import generate_username
...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    def check_user_role(self, request):
        if request.user.is_authenticated:
            return Response({"role":"is_superuser",},status=status.HTTP_200_OK)
        ...
        try:
            ...
            return Response({
                'error': 'memebership didt found '
            }, status=status.HTTP_404_NOT_FOUND)
        except ProjectMembership.MultipleObjectsReturned:
            return Response({'error' : "multiple role retruned"},status=status.HTTP_400_BAD_REQUEST)
